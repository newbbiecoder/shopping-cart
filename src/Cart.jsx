...
export default function Cart() {
    ...
    return (
        <section className="pd-15 lg:p-15 lg:pl-35">
            ...
            <div className="flex flex-col lg:flex-row justify-between">
                <div className="flex flex-col gap-7 flex-wrap">
                    {cartItems.map((item) => 
                        <><div className="flex gap-9 items-center justify-start flex-wrap">
                            <div className="flex gap-4 pl-3 lg:pl-0">
                            ...
                            <div className="flex gap-4 lg:gap-7 pb-8 ">
                                <div className="flex gap-2 lg:gap-4 flex-wrap justify-start items-start">
                                    <p className="w-9 h-9 flex items-center justify-center border border-gray-400 rounded-lg text-gray-500 font-bold text-lg cursor-pointer select-none ml-1 lg:ml-0" onClick={() => decrement(item.id)}>-</p>
                                    <p className="text-lg w-10 lg:w-20 h-9 flex items-center justify-center border border-gray-400 rounded-lg">{item.qty}</p>
                                ...
                                <div className="flex gap-3 flex-wrap relative">
                                    <p className="pt-1 font-bold text-xl inline-block">${(item.price * item.qty).toFixed(2)}</p>
                                    <button className="flex gap-2  pt-1 cursor-pointer group ml-2 lg:ml-0" onClick={() => handleRemove(item.id)}>
                                        <FaRegTrashAlt className="w-4 h-4 mt-1 fill-red-500 group-hover:fill-red-600 " />
                                        <p className="text-red-500 group-hover:text-red-600">Remove</p>
                                    </button>
                                </div>
                            </div>
                ...
                {cartItems.length !== 0 && (
                    <div className="flex flex-col p-7 mr-auto ml-auto border border-gray-400 rounded-lg w-80 h-fit mt-5 lg:mt-0">
                        <h2 className="text-3xl font-semibold pb-2">Order Summary</h2>
                        <div>
                            <p className="text-gray-600 text-lg flex justify-between pt-2 pb-1">Subtotal <span className="text-black">${getSubtotal().toFixed(2)}</span></p>
